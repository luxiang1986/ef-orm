...
public class LimitHandlerTest {
	BindSql[] sqls = { new BindSql("SELECT  \n" + "T.NAME AS PNAME, T1.NAME FROM 	parent T, 	child T1 WHERE	T.ID = T1.PARENTID order by t1.name",null),
			
			new BindSql("(select * from child t where t.code like 'code%') union all (select rootid as parentid,code,id,name from parent) union all (select * from child t) order by name ",null) };
...
"\nORDER BY name";
	...
	private void doAssert(BindSql raw, LimitHandler lh, String expect) {
